...
2. **Selesai.** **Abstraksi LLM provider.** Deliverable: `LLMProviderInterface`, `LLMProviderFactory` (OpenAI, Anthropic, OpenRouter), migration `llm_configs` (termasuk `supports_vision`, flag manual yang di-set owner — tidak dideteksi otomatis dari nama model), endpoint konfigurasi untuk owner. Ditambahkan di luar cakupan awal: `POST /api/v1/me/llm-config/describe-image`, yang mengirim foto produk yang sudah diunggah ke provider vision yang dikonfigurasi dan mengembalikan draf deskripsi produk — dipicu manual oleh tombol yang diklik owner (bukan otomatis di setiap upload foto), karena setiap panggilan adalah request berbayar ke akun provider milik owner. Dibatasi oleh `RateLimiter` yang sama dengan key `(node_id, 'llm_call')` sesuai Bagian 4.3. Belum ada permukaan chatbot yang menghadap visitor.
