...
| 6.0 Payment layer | **Sebagian besar selesai** | Interface, factory, dummy gateway; **Paywuz** (create transaction + webhook HMAC-SHA256) dan **Midtrans** (Snap create + Core API status/cancel/refund + webhook SHA512) sungguhan sudah jalan dengan persistensi `payments`/`payment_transactions` dan idempotency; pengaturan admin gateway belum ada |
...
| 10.0 Testing, security, deployment | **Sebagian selesai** | 25 test scripts; GitHub Actions CI; panduan Dokploy (EN & ID); audit trail di 3 service; catatan: migration nyata di `database/migrations/` belum pernah tervalidasi jalan di SQLite (semua test menulis skema minimal sendiri, lihat §4) |
...
| Payment lifecycle | Dummy create/status/cancel/refund/normalisasi webhook; **Paywuz** (create + webhook HMAC-SHA256) dan **Midtrans** (Snap create + status/cancel/refund + webhook SHA512) sungguhan, keduanya dengan idempotency dan persistensi `payments`/`payment_transactions` | `getPaymentStatus`/`cancelPayment`/`refundPayment` untuk Paywuz (tidak terdokumentasi di API mereka), UI pengaturan admin gateway, penanganan refund pasca-PAID (tercatat tapi tidak mengubah status), rekonsiliasi |
...
- **Administration dashboard sungguhan:** endpoint backend untuk settings, integrasi, produk, order, payment, analitik. Federation (satu dari 10 menu mockup) sudah sebagian — summary + capabilities di atas — sisanya (Overview, Payments config, Analytics, Settings appearance/language/security) belum digarap.
...
3. **Administration dashboard backend** — endpoint untuk Overview (ringkasan metrik), Settings (appearance/language/node config), dan Payments config; sambungkan tabel `payment_gateway_configs` yang masih sama sekali belum dipakai kode apa pun (kredensial gateway saat ini cuma lewat `.env`). Federation (summary + capabilities) sudah selesai di atas.
