...
- **Administration dashboard (real):** the dashboard exists only as a static mockup; no backend endpoints, views, or auth-gated screens for settings, products, orders, payments, or analytics. Federation (one of the mockup's 10 menus) is now partly done — summary + capabilities above — the rest (Overview, Payments config, Analytics, Settings appearance/language/security) is not.
...
3. **Administration dashboard backend** — API endpoints for Overview (summary metrics), Settings (appearance/language/node config), and Payments config; wire up the `payment_gateway_configs` table, which is still completely unused by any code (gateway credentials only come from `.env` today). Federation (summary + capabilities) is now done, above.
