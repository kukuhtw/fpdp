...
3. **Paid CV/resume access.** Deliverable: `cv_documents`, `cv_access_grants` migrations; upload endpoint; gated read endpoint reusing `PaymentGatewayInterface`. The simplest paywall — good end-to-end proof of visitor identity + payment before the more complex chatbot.
