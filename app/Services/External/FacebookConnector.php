...
final class FacebookConnector implements ExternalContentProviderInterface
{
    ...
    public function __construct(private readonly array $configuration=[]){$requester=$configuration['http_requester']??null;$this->requester=$requester instanceof Closure?$requester:static fn(string $url,array $headers):array=>(new HttpClient())->get($url,$headers);}
    ...
    public function fetchPosts(array $account,?string $cursor=null):array
    {
        ...
        try{$response=($this->requester)($url,['Authorization'=>'Bearer '.$token,'Accept'=>'application/json']);}catch(\Throwable $e){return['items'=>[],'next_cursor'=>null,'error'=>$e->getMessage()];}
