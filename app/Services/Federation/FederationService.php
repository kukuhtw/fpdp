    ...
    public function listPublicByHandle(string $handle, array $query = []): array
    {
        ...
        $items = array_map(function (array $row) use ($latestPosts): array {
            ...
            return [
                ...
                'actor' => [
                    ...
                    'federated_address' => $row['federated_address'],
                    'display_name' => $row['actor_display_name'],
                    'avatar_url' => $row['actor_avatar_url'],
                    'canonical_url' => $row['actor_canonical_url'],
                    'node_domain' => $row['node_domain'],
                    'node_name' => $row['node_name'],
                ],
                'relationship_status' => $row['relationship_status'],
                'latest_post' => $latestPost !== null ? [
                    'id' => $latestPost['public_id'],
                    'title' => $latestPost['title'],
                    'content' => $latestPost['content'],
                    'canonical_url' => $latestPost['canonical_url'],
        ...
        return [
            ...
            'next_cursor' => $hasMore && $last !== null
                ? rtrim(strtr(base64_encode((string) $last['id']), '+/', '-_'), '=')
                : null,
            'has_more' => $hasMore,
        ];
    }
